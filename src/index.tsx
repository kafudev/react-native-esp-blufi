...
const { RNEspBlufi } = NativeModules as { RNEspBlufi: EspProvisioning };

const eventEmitter: EspEventEmitter = new NativeEventEmitter(RNEspBlufi as any);

export default RNEspBlufi;
  ...
  useEffect(() => {
    ...
    async function _handleAppStateChange(nextAppState: AppStateStatus) {
      ...
      if (nextAppState === 'active') {
        ...
        const result = await RNEspBlufi.checkPermissions();
        console.log(`checkPermissions result: ${result}`);
        if (result && !isConnecting.current && currentStep === 1) {
          console.log('Start BleScan');
          RNEspBlufi.startBleScan(devicePrefix);
        }
      }
    }
    let eventAppState = AppState.addEventListener(
      'change',
